...
class ParamLayer(BaseLayer):
    def __init__(self, w_shape, activation, w_initializer, b_initializer, use_bias):
        ...
        if w_initializer is None:
            nn.init.TruncatedNormal(0.0, 0.01).initialize(self.w)
...
class Conv2D(ParamLayer):
    ...
    def forward(self, x):
        ...
        wrapped_out = self._wrap_out(
            ...
            else self._activated.transpose((0, 3, 1, 2))
        )
        return wrapped_out
    ...
    def backward(self):
        ...
        for i in range(dz.shape[1]):
            for j in range(dz.shape[2]):
                padded_dx[:, i * s0 : i * s0 + k0, j * s1 : j * s1 + k1, :] += (
                    dz[:, i, j, :]
                    .reshape((-1, self.out_channels))
                    .dot(t_flt.reshape((self.out_channels, -1)))
                    .reshape((-1, k0, k1, padded_dx.shape[-1]))
                )
        t, b, l, r = [
            self._p_tblr[0],
            padded_dx.shape[1] - self._p_tblr[1],
            self._p_tblr[2],
            padded_dx.shape[2] - self._p_tblr[3],
        ]
        self.data_vars["in"].set_error(
            padded_dx[:, t:b, l:r, :]
        )  # pass error to the layer before
    ...
    def convolution(self, x, flt, conved):
        ...
        for i in range(0, conved.shape[1]):  # in each row of the convoluted feature map
            for j in range(
                0, conved.shape[2]
            ):  # in each column of the convoluted feature map
                x_seg_matrix = x[
                    :, i * s0 : i * s0 + k0, j * s1 : j * s1 + k1, :
                ].reshape(
                    (batch_size, -1)
                )  # [n,h,w,c] => [n, h*w*c]
                flt_matrix = t_flt.reshape(
                    (-1, flt.shape[-1])
                )  # [h,w,c, out] => [h*w*c, out]
                filtered = x_seg_matrix.dot(
                    flt_matrix
                )  # sum of filtered window [n, out]
                conved[:, i, j, :] = filtered
        return conved
    ...
    def fast_convolution(self, x, flt, conved):
        ...
        wx = self._padded_col.dot(
            w_t
        )  # [n*oh*ow, fh*fw*c] dot [fh*fw*c, oc] => [n*oh*ow, oc]
    ...
    def fast_backward(self):
        ...
        dw = self._padded_col.T.dot(dz_reshape).reshape(
            self.kernel_size[0], self.kernel_size[1], -1, self.out_channels
        )
        ...
        self.data_vars["in"].set_error(
            padded_dx[:, t:b, l:r, :]
        )  # pass the error to the layer before
        return grads


class Pool_(BaseLayer):
    def __init__(
        self, kernal_size=(3, 3), strides=(1, 1), padding="valid", channels_last=True
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        wrapped_out = self._wrap_out(
            out if self.channels_last else out.transpose((0, 3, 1, 2))
        )
        return wrapped_out
...
class MaxPool2D(Pool_):
    def __init__(
        self, pool_size=(3, 3), strides=(1, 1), padding="valid", channels_last=True
    ):
        super().__init__(
    ...
    def backward(self):
        ...
        for i in range(dz.shape[1]):
            for j in range(dz.shape[2]):
                ...
                window_dz = dz[:, i : i + 1, j : j + 1, :] * window_mask.astype(
                    np.float32
                )
        ...
        for i in range(dz.shape[1]):
            for j in range(dz.shape[2]):
                window_dz = dz[:, i : i + 1, j : j + 1, :] * np.full(
                    (1, k0, k1, dz.shape[-1]), 1.0 / (k0 * k1), dtype=np.float32
                )
                padded_dx[:, i * s0 : i * s0 + k0, j * s1 : j * s1 + k1, :] += window_dz
        t, b, l, r = [
            self._p_tblr[0],
...
class Dropout(BaseLayer):
    ...
    def forward(self, x):
        self._x = self._process_input(x)
        if self.training:
            self._keep_mask = (np.random.rand(*self._x.shape) < self.keep_p).astype(
                "float"
            )
            out = self._x * self._keep_mask
        else:
            out = self._x * self.keep_p
        return self._wrap_out(out)

    def backward(self):
        dz = self.data_vars["out"].error
        grad = None
        self.data_vars["in"].set_error(dz * self._keep_mask)
        return grad
...
def get_padded_and_tmp_out(img, kernel_size, strides, out_channels, padding):
    ...
    if padding == "same":
        ...
        raise ValueError
    padded_img = np.pad(
        img, ((0, 0), (pt, pb), (pl, pr), (0, 0)), "constant", constant_values=0.0
    ).astype(np.float32)
